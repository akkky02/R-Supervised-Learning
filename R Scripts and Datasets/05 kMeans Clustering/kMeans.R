...
utilities.df.scale <- scale(utilities.df[1:7,])
View(utilities.df.scale)
...
kmu3 <- kmeans(scale(utilities.df), 3, nstart = 100)
print(sort(kmu3$cluster))

print(kmu3$centers)
print(dist(kmu3$centers))

# plot an empty scatter plot
plot(c(0), xaxt = 'n', ylab = "", type = "l", 
     ylim = c(min(kmu3$centers), max(kmu3$centers)), xlim = c(0, 8))
...
for (i in c(1:3))
  lines(kmu3$centers[i,], lty = i, lwd = 2, col = ifelse(i %in% c(1, 3, 5),
                                                        "black", "dark grey"))

# name clusters
text(x = 0.5, y = kmu3$centers[, 1], labels = paste("Cluster", c(1:3)))
...
fviz_cluster(kmu3, data = utilities.df)
